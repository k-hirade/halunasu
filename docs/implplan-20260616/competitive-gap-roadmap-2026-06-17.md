...
| #1段階B レセ電(UKE) | ✅ 完了 | `fee-core` `buildReceiptDenshin`(IR/RE/HO/KO/SN/SI/IY/TO/CO/SJ・和暦/診療識別/負担区分/レセプト種別マッピング)+`serializeUke`(カンマ区切り/CRLF)。`fee-api` `GET …/receipt.uke?encoding=shift_jis|utf-8`(既定SJIS, iconv-lite)。fee-web に文字コード選択+「レセ電(UKE)出力」。テスト+3。⚠ 診療識別/負担区分/レセプト種別/各レコード項目は令和8記録条件仕様での**フィールド単位検証が実請求前に必須** |
