...
- UIカード上の操作は `条件を確認` に集約する。採用・保留・算定しないの判断は、根拠と条件を読める詳細ポップアップ内で行う。
...
5. 提案カードは `条件を確認` だけを表示し、詳細ポップアップ内で `算定する +○点` / `保留` / `算定しない` を選べるようにする。
...
- 増点提案カードは `条件を確認` に集約され、詳細ポップアップ内で採用・保留・算定しないを判断できる。
...
今後の診療報酬算定は、個別のSOAPを解決するための条件分岐ではなく、以下の一般化されたパイプラインに寄せる。

```text
カルテ本文
→ 医療イベントとして構造化
→ マスターで候補検索
→ 算定ルールで採否/要確認/増点提案に分類
→ ユーザーが条件を確認して採用
```
...
| --- | --- | --- | --- |
| 医療イベント構造化 | カルテ本文から、実施済み・処方済み・予定・指導のみ・既往を分ける | LLMと汎用ルールで `billing_events` / `excluded_events` を作る | 点数や診療報酬コードを決め打ちする |
| マスター候補検索 | イベント名・同義語・証拠文から標準コード候補を探す | 複数候補を返し、確信度が低いものは要確認にする | 特定SOAPの語句だけを直接コードへ結びつける |
| 算定ルール分類 | 候補を `算定中` / `増点提案` / `確認・修正` に分類する | 実施済みなら候補化、条件付きなら提案、予定なら保留にする | 条件不明の項目を自動確定する |
| ユーザー採用 | 医療事務が条件を確認して採否を決める | `算定する +○点` / `保留` / `算定しない` を保存し合計へ反映する | 条件説明なしに点数だけを足す |

### 個別SOAP過学習を避ける実装ルール

- 疾患名や薬剤名を増やすだけの修正は原則禁止する。
- 新しい診療科・疾患で漏れが出た場合は、まず「イベント種別」「実施/予定/指導のみ」「マスター検索語」「算定条件」のどの層で失敗したかを特定する。
- コード内に同義語を持つ場合も、特定症例の穴埋めではなく、マスター検索を助ける汎用辞書として扱う。
- 具体例はLLMプロンプトに含めてよいが、例をコピーするのではなく、任意の診療科で同じスキーマへ抽出するよう明記する。
- テストは「アトピーだけ」「CA125だけ」ではなく、同じ仕組みで別の検査・薬剤・管理イベントが解けることを確認する。

### 今回の実装方針

- ルールベースの検査提案は、`IgE` / `好酸球` などの固定リストではなく、客観所見の文からマスター検索語を作る。
- 薬剤抽出のフォールバックは固定薬剤リストではなく、薬剤らしい接尾辞・用量表記・処方文脈から候補名を作る。
- LLMが返した `management` / `counseling` イベントは、コード確定できない場合でも「医学管理料・指導料の確認」として汎用の増点提案または確認項目に残す。
- 採用済み明細と同じ薬剤・検査に対する数量不足警告は、特定薬剤名ではなく対象名で重複抑制する。

### 2026-06-07 追加修正方針

- 同じFee session内でカルテ本文を差し替えた場合、読み込み時点の手入力病名を新しいカルテへ持ち越さない。
  - フロントは、読み込み後に病名欄を触っていない状態でカルテ本文が変わったら、病名を自動抽出扱いへ戻す。
  - API側も、カルテ本文が変わったのに前回と同じ手入力病名がそのまま送られた場合は、古い入力の持ち越しとして破棄する。
- 管理料提案は「本文に慢性疾患らしい単語がある」だけでは出さない。
  - 対象疾患に明確一致: 増点提案。
  - 対象外: 出さない。
  - 対象か不明: 確認・修正へ下げる。
  - 急性疾患病名しかない: 管理料提案は禁止。
- 確認・修正項目は、医療事務が原因を見分けられるように分類する。
  - `施設設定`: 施設基準・届出。
  - `病名・コメント`: 適応病名、レセプトコメント。
  - `薬剤情報`: 日数、総量、1回量、1日回数。
  - `マスター確認`: 標準コード候補、マスター検索が必要なもの。
  - `実施確認`: 実施済み/予定/依頼/指導のみの切り分け。
  - `入力不足`: 必須情報が空欄または不足。
  - `算定条件`: 上記以外の算定ルール確認。
- アレルギー検査は、個別症例ではなく検査種別として抽出する。
  - 非特異的IgE、特異的IgE、皮膚プリック/皮膚反応テストを別候補としてマスター検索する。
  - 当日実施か過去結果か不明な場合は要確認に留める。
...
- 構造化された採用可能提案は詳細ポップアップ内で `算定する +○点` と表示できる。カード上は `条件を確認` に集約する。
