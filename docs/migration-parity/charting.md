...
2026-05-29から2026-05-30の復元作業で旧Next.js資産を `apps/charting-web` に取り込み、旧E2Eはローカルで通過した。STG/PRODのNetlifyはNext.js配信へ切り替え済みで、`/sessions`、Core seeded operator login、`/api/v1/operator/me`、`/api/v1/sessions` まで独自ドメイン上で確認済み。

旧 `services/gateway` は `services/charting-gateway` として取り込み開始。旧 `services/finalize` と旧 `services/billing` も、それぞれ `services/charting-finalize-legacy`、`services/billing-api-legacy` として保存した。`services/charting-gateway` にはCore Platformのlogin identity/member/entitlementを読む短期認証bridgeを追加済み。次の作業は、認証以外のshared master dataもCoreへ寄せること。
...
| `/sessions/[sessionId]` | ルート復元済み | 実セッション詳細のSTG/PROD操作確認 |
| スマホ録音 | 旧UI/API保存済み | Gateway deployとCore bridgeが必要 |
| live STT | 旧実装保存済み | OpenAI/Deepgram secrets、費用制御、STG smokeが必要 |
| final transcript | 旧実装保存済み | `charting-finalize-legacy` を本番workerへ昇格するか統合 |
| SOAP生成 | 旧実装保存済み | OpenAI SOAP生成、プロンプトプロファイル、再生成、レビュー、承認をSTGで確認 |
| admin SOAP format | 旧UI/API保存済み | Core Adminとの権限/導線整理 |
| audio test | 旧UI/API保存済み | PC/モバイル音声テストをSTGで確認 |
| billing | 旧実装保存済み | Core billing/Stripe entitlementへ接続 |
| E2E | ローカル移植済み | deploy後の録音/SOAP操作確認を追加 |
| 旧静的Charting | 削除済み | 誤配信防止のため復活させない |
| Gateway認証 | Core Platform認証bridge追加済み | STG/PRODでCore seed userによるlogin確認 |
...
- `/sessions` と `/sessions/[sessionId]` が旧UIとして表示される。`/sessions` はSTG/PROD Done、詳細操作は深掘りPending.
...
- STG/PRODでログイン、一覧、API proxyは確認済み。
- STG/PRODで新規セッション、詳細、録音導線、SOAP生成/再生成/承認まで確認する。Pending.
