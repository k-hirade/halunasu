...
部分完了。現行 `apps/lp` は旧 `medical-lp` のファイルを多く含むが、`index.html` と `README.md` に差分があり、現行側には `signup.html` とvalidate scriptが追加されている。Netlify静的配信はSTG/PRODへ反映済みで、PROD `halunasu.com` は200を確認済み。LPは他アプリより移行度が高いが、旧デザイン/文言/フォーム導線の完全一致確認は未完了。
...
| signup | 現行追加 | Core signupに接続しているか確認 |
...
- 追加した `signup.html` がCore signup APIへ接続され、STG/PRODで実行確認済み。
