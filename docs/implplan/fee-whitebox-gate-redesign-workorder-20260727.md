...
**2026-07-28 追記(重要)**: S1〜S4適用後のSTG計測(00196)で
joint eligible 10.2%まで改善したが、続く `c6d7d78`(00197)で
**0.0%へ後退**した。原因はS1〜S4ではなく、WX3分類器の入力が
行テキストから節テキストへ変わり訓練契約から乖離したこと。
復旧と再発防止は
[分類器入力契約ワークオーダー(R1〜R4)](./fee-whitebox-context-input-contract-workorder-20260728.md)
を参照。**S系の再計測はR1適用後に行うこと**(現状のまま測っても
context側が全通過を潰すため無意味)。
