...
- 作成日: 2026-08-03
- status: Sidecar患者別handling保存とB1を実装済み / 会計確定・fee-web管理・B2は未実装
...
| --- | --- | --- |
| 患者別交通費 | `patient_charge`として保険算定から分離。未設定時はSidecarで患者別handlingを選び、Fee API経由で `fee_patient_charge_contracts` へ保存する | 施設既定値、実費金額、同意、会計・領収証連携、fee-webの参照・管理UI |
| タブ切替中の算定 | 抽出完了後のFee API処理を継続し、別患者には表示せず、元カルテ復帰時だけ結果を復元するB1 | Side Panel閉鎖・再読込後まで復旧するB2 |

したがって、質問への回答は次のとおり。

1. **患者別交通費のSidecar保存フローを実装した。** 現段階で同じ値を読むのはFee APIとSidecarであり、fee-web接続は次フェーズである。
...
`施設設定を継承` は選択・保存できるが、施設既定値はまだ実装していないため、現時点の解決結果は必ず未確定である。fee-webのGET/更新API、同意文書、金額確定、会計・領収証も今回の範囲外である。
...
`handling` は `inherit|charge|waive|included_in_contract` とする。契約IDは組織・施設・患者・charge typeから決定的に生成し、revision付き `settingEvents` へ履歴を追記する。同意状態、施設policy revision、別docによる新版/supersedeは将来拡張であり、今回の実装には含めない。

### T2: 解決規則（1-4の一部を実装済み）

1. canonical patientが未解決なら `patient_not_linked`、自動請求しない。
2. 診療日に有効な患者契約を取得する。
3. 患者契約が `waive` / `included_in_contract` なら、患者への請求対象外であるhandlingを返す。会計金額はまだ生成しないため `amountYen` は `null` のままとする。
4. `charge + amountMode=actual + 金額未入力` は `pending_actual` とし、自動請求しない。同意判定は未実装である。
5. 有効契約がなければ `setting_not_configured` とする。施設方針の解決は未実装であり、`inherit` も `facility_default_not_configured` となる。
...
Sidecarが更新できるのはhandlingだけである。適用開始日はdraftの診療日に固定し、終了日は設定しない。`charge` は `amountMode=actual`・金額未確定に固定する。直接APIを呼んでも固定金額、任意の開始日・終了日を指定できず、金額確定や過去履歴の訂正は将来のfee-web/APIへ分離する。
...
- Sidecar: 点数候補とは別の「患者負担」1行にドロップダウンを置き、`施設設定を継承 / 請求する / 請求しない / 契約内に含む` を患者別に設定する。
- 患者設定も施設設定もない場合は `未設定` とし、選択・保存までは請求へ含めない。`未設定` を0円や請求なしへ変換しない。
...
9. 交通費handling保存中は同じカルテの再算定を開始させず、算定中も保存を開始させない。タブ切替中に保存が失敗した場合も、エラーを元taskへ保持して復帰時に表示する。
10. このフェーズではService Worker、サーバAPI、PHIの永続化を増やさない。

受入テストは、算定中タブ切替、切替後成功、元タブ復帰、入力snapshot復元、response identity不一致のfail closed、抽出中切替のfail closed、複数カルテtaskの保持、交通費保存との排他、別患者への誤表示0件を含める。
