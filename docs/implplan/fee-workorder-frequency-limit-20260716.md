...
### 2026-07-22 L7実測による補足

1002の電話カルテを月次評価ハーネスで`outpatient`へ分離した結果、訪問診療料の誤追加は止まった。
一方、既存UKEの`112007950`電話等再診料は候補・確定のどちらにも出ず、通常の
`112007410`再診料と`180820010`物価対応料が選択された。大区分`setting=outpatient`だけでは
電話再診と通常外来を区別できないため、本節の`encounterDetails.visitKind=telephone_revisit`と
電話等再診料候補化が必要であることを実測で確認した。

同一建物については、月次ハーネスが患者情報から決定論入力したことで、施設患者の全45訪問明細で
`114030310 / 180726010`、個人宅の全12訪問明細で`114001110 / 180725910`が選択され、
区分未確定候補は0件だった。詳細は
`docs/20260722-longitudinal-l7-rerun-20260722_202259/README.md`を参照する。
