...
レビュー4指摘の修正を含め仕様どおりであることを確認した。残修正1件:

### F1. [P2] 電話語彙検知に過去・外部文脈の除外がない
...
修正: 同ファイル内の `hasCurrentPerformedActPredicate` が既に使っている
過去・外部文脈除外(`isPastOrExternalClinicalServiceContext` 等)と同じ正規化・除外を
`hasTelephoneVisitWording` にも適用し、上記2文を反例テストとして追加する
(「本日電話等再診」「家族から電話相談あり電話にて指示」は発火を維持)。
