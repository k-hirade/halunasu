...
- **回帰=standing fixture再走**(2026-07-24 shadow計測で期待値を実マスタに訂正):
  対象2ペア(管理料×喀痰吸引、管理料×人工呼吸)は実マスタで**特例区分=1**であり、
  resolutionは `auto_winner` ではなく **`conditional_review`**(自動ブロックしない)。
  したがって正しい期待動作は:
  - 喀痰吸引が抽出された反復: same_monthのconflict 1件が `conditional_review`/
    未解決として検知され、**当該月のエクスポートがブロック**される
    (shadow計測 20260724_191240 でconflict検知はSTG実証済み)。
  - 人の解決(例: `choose_a`=管理料側)を保存すると喀痰吸引occurrenceがblockedになり、
    確定点数が3,502点に揃う。**点数の反復一致は自動ブロックではなく
    「未解決→人の解決」を経て達成される**のがこのペアの正しい姿。
  - 人工呼吸302点の候補は `conditional_review` 未解決時のcandidateMustWaitにより
    候補点数へ計上されない(候補50点で反復一致)。
  - 3コード(管理料+喀痰吸引+人工呼吸)が同月に揃った反復があれば複雑成分となり、
    自動適用なし+出力ブロック(これも正)。
  enforce受入のため、評価ハーネスに①月次レコードへのexclusion指標記録
  (exclusionMode/constraintsStatus/conflicts/blockedLines(Preview)/unresolved)と
  ②検知したconflictへ指定actionを解決APIで保存するステップ、の2拡張が必要。
...
1. standing fixture再走(enforce+解決ステップ込み)で確定点数・候補点数が
   3反復完全一致。喀痰吸引反復は「未解決→エクスポート不可→解決後に一致」の
   経路を通ること。
...
## 受入結果 (2026-07-24 STG実測)

- shadow計測(`docs/20260724-monthly-exclusion-shadow/20260724_191240/`):
  mode=shadow・constraintsStatus=complete、喀痰吸引が出た反復のみconflict 1件
  (same_month/conditional_review)を検知、提出集合は不変。サーバログで確認。
- enforce計測(`docs/20260724-monthly-exclusion-enforce/20260724_193413/`)+
  解決APIプローブ(反復3の実患者に対する実API操作)で以下を全て実証:
  1. 未解決conditional_review検知(allowedActions 4種、自動ブロックなし=特例区分1の正)
  2. 未解決中のCSVエクスポート409(`receiptDraft.unresolvedExclusionCount`が理由に含まれる)
  3. `choose_a` の解決保存200(CSRF・許可action検証・監査ログ経路)
  4. 解決適用: 3,550→3,502点、140003810がblockedLinesへ、unresolved=0
- 解決後もエクスポートは409だが、理由は合成fixture固有の既存必須項目
  (医療機関コード・保険者番号・connector spec・未承認候補)であり背反とは独立。
  背反issue自体は unresolved=0 から決定論的に消えており、多重ゲートの独立性も確認できた。

**X1〜X5の受入は完了**。残作業は評価ハーネスの2拡張(月次レコードへのexclusion指標
記録/conflictへの指定action保存ステップ)のみで、これが入れば受入シナリオを
プローブなしで再現できる。
