...
1. **コードベース分離**: 拡張はサーバ実装から分離し、共有するのは「APIの契約」だけ。
   E2E回帰にはmock_homisを使う(こちらも既にhalunasu外)。
   **【2026-07-18改訂】当初の「別リポジトリ」は、halunasu内の専用ディレクトリ
   `clients/homis-sidecar/` に変更**(build workorder参照)。分離の本体はAPI契約
   (サーバ側で強制済み)であり、リポジトリ分離は手段にすぎない。単独リポジトリは
   fixture・契約定義・CIの重複と契約変更時の2リポジトリ同期コストを生むため、
   ディレクトリ分離+依存ガード(独立package.json / 内部import禁止のgrepテスト /
   .gcloudignoreでビルドコンテキスト除外)で同じ効果を得る。
   単独プロダクト化・外部チームへの引き渡しが必要になった時点でリポジトリ分割を再検討する
   (ガードにより分割コストは移動のみ)。
...
- **【2026-07-18確定】最初からデバイス認可方式**: 拡張がuserCodeを表示→fee-webの
  承認ページ(ログイン+MFA済み)で承認→拡張がポーリングで長寿命grant+短寿命トークンを取得。
  パスワードは拡張に一切触れない。詳細仕様は build workorder S2。
