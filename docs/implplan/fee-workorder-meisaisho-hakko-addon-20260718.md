...
### 2026-07-18 STG実測で判明した入力制約

現行mock HOMISデータの患者1012は、`charts.jsonl`上では定期訪問2件と往診1件であり、
A001再診料に対応する外来受診が含まれていない。既存UKEには112015770が1回あるが、対応する
外来カルテまたは受診区分が再算定入力に無いため、このデータだけでM4-1の一致判定はできない。

代替のデプロイスモークとして患者1001を`outpatient`固定で実行し、初診では非付与、同月2回目の
再診で112015770が1回・1点となることを確認した。非blockedの算定line itemを明示承認・保存した後の
患者月次にも同コードが1回・1点で反映された。在宅・往診15受診への誤付与は0件だった。
最終的なM4-1は、実際の外来再診カルテと受診区分を入手した後に再実施する。
