...
## L7月次Runで観測した全文抽出の揺れ (2026-07-22)

縦断コンテキストL7の初回月次Run
(`docs/20260722-longitudinal-l7-mock-20260722_185226/`)では、mock HOMIS由来の
5患者・各4受診・3反復、計60算定を同一revisionで実行した。次の変動を実測した。

- 受診別の臨床イベント数が反復間で一致した受診は **0/20**。
- 患者単位の確認事項集合が反復間で一致した患者は **0/5**。
- 1002では`140009310 人工呼吸`の月内検出回数が **1/2/3回**となり、候補点数が
  **302/604/906点**へ変動した。
- 1011では`140003810 喀痰吸引`が3反復中1反復だけ出現した。
- 全60算定でextraction memoは有効だったが、`memoUsedVisitCount=0`、
  `memoHitLineRatio=0`だった。したがって上記変動はメモ再利用差ではなく、
  全文LLM抽出の非決定性に帰属する。

このRunは受診区分・同一建物入力が不足していたため、確定点数やUKE再現率の合否には使わない。
一方、同一本文の全文抽出が揺れるという観測自体は入力区分不足とは独立しており、本チケット群の
抽出安定性課題の実測根拠として扱う。L7縦断メモの合否からは分離する。

実運用のcopy-forwardカルテでは、継続行がメモに命中すれば変動範囲は主に新規行へ限定されると
見込まれる。ただし、これは現時点では設計上の見立てであり、顧客カルテにおけるcopy-forward率と
新規行の抽出安定性を測るまでは、実効改善率として扱わない。

### ハーネス修正後Run

受診区分・同一建物を補正した再測定
(`docs/20260722-longitudinal-l7-rerun-20260722_202259/`)でも、同じ構造の揺れが残った。

- 月次確定点数は4/5患者、候補点数は4/5患者で反復一致した。
- 確認事項集合は0/5患者、臨床イベント数は1/20受診だけが反復一致した。
- 1002は反復2だけ`140003810 喀痰吸引`48点が確定へ追加され、人工呼吸候補回数も変動した。
- 1010の6月7日はイベント数が`0 / 3 / 14`となり、最大差14だった。
- 全60算定は`extractionMode=full`、メモ使用0件、空抽出ガード発火0件だった。

入力区分を修正しても全文抽出の揺れは解消しないことが再確認された。これはL7のメモ実装回帰ではなく、
全文LLM抽出の安定性課題として扱う。なお、1002の喀痰吸引は「吸引管理」の記載を当日手技へ広く解釈した
過抽出であり、空抽出ガードを広げる理由にはしない。
