# Fee SOAP E2E v2 1000ケース blueprint-first 手書き拡張計画
...
2026-06-12時点の方針変更: SOAP本文を先に増やさない。まず1000ケース分の算定ゴールドblueprintを作り、その後に各blueprintへ1件ずつ手書きSOAPを対応させる。
...
`data/tests/fee-soap-e2e-v2/gold-blueprints.json` を1000ケースの母集団とする。各blueprintは以下を持つ。

- assertion level: exact / review_required / safety / unsupported_expected / split_required
- expectedCalculation: exactなら候補コード・合計点、非exactならneeds_review
- expectedExtraction: requiredBillingSignals / requiredReviewTopics / forbiddenCandidates
- expectedClaimContext: 点数再現に必要な受診・施設・検査・薬剤・処置コンテキスト
- facilityFixtureKey: 施設基準・機器・電子保存など、カルテ本文に書かない入力事実
- requiredClinicalAnchors: SOAP本文に自然な臨床文として必ず入れる根拠
- distractorRequirements: 過去値、他院結果、予定、見送り、市販薬などの邪魔情報

blueprintはSOAP本文ではないため、coverage matrixからの計画生成を許容する。ここで自動生成してよいのは「算定設計の枠」であり、臨床記録本文ではない。
...
- 施設属性をカルテ本文に書く。
- blueprintの `requiredClinicalAnchors` を無視して、期待コードだけを入れる。

## バッチ計画

既存51件を土台に、まず1000件のblueprintを固定する。その後、残り949件のSOAP本文を20〜25バッチで追加する。
...
```bash
npm run generate:fee-soap-e2e-v2:blueprints
...
- 1000件のblueprintが存在しても、SOAP本文が1000件あるという意味ではない。
- `status=blueprint_ready_chart_pending` はSOAP手書き待ち。
- `status=chart_authored` はSOAP本文まで作成済み。
