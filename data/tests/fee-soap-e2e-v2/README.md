...
画像系ケースでは、カルテ本文に請求処理の都合を書かず、カルテ記載と施設fixtureを分けて評価する。

| 属性 | カルテ/fixtureの状態 | 期待する扱い |
| --- | --- | --- |
| 造影 | 造影への言及なし | 非造影として扱い、造影確認topicは要求しない |
| 造影 | 造影剤使用が明記 | 造影ありとして扱う |
| 造影 | 造影の有無が記載上不確定 | `造影確認` topicを要求する |
| 電子画像管理 | fixtureで電子画像管理あり | 電子画像管理加算を期待する |
| 電子画像管理 | fixtureで電子画像管理なし | 加算なし。電子保存確認topicは要求しない |
| 電子画像管理 | fixtureがunknown | `電子保存確認` topicを要求する |
| CT/MRI機器区分 | fixtureで機器区分あり | 該当区分で点数確定を期待する |
| CT/MRI機器区分 | fixtureがunknown | `機器区分確認` topicを要求する |

`算定へ反映`、`算定に反映`、`自動確定`、`会計前に確認` のようなシステム語彙はカルテ本文に書かない。
...
LLM抽出の揺れは、同一ケースを3回実行したときのケース単位一致率で監視する。候補コード集合・合計点・失敗stageが一致するケースの割合が90%未満になった場合、P2として高リスク/未解決checklist itemだけを対象に検証LLMまたは並列自己一致を検討する。全件常時2層化は行わない。
