...
test("member permissions screen exposes safe action modals", { timeout: 60_000 }, async () => {
  await withPage(async (page) => {
    ...
    await page.getByRole("heading", { name: "ログイン用パスワード再設定" }).waitFor({ state: "visible" });
    await page.getByRole("button", { name: "安全なログイン用パスワードを作る" }).click();
    const generated = await page.getByLabel("新しいログイン用パスワード").inputValue();
