...
export function AdminNavProvider({ children }) {
  ...
  const closeAdminNav = useCallback(() => setIsOpen(false), []);
  const openAdminNav = useCallback(() => setIsOpen(true), []);
  const toggleAdminNav = useCallback(() => setIsOpen((current) => !current), []);
