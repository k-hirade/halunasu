...
test("calculate request snapshot keeps the v1 sidecar boundary", () => {
  ...
  assert.match(apiSource, /\/patient-charge-setting/);
  assert.match(apiSource, /setPatientChargeSetting/);
  assert.match(apiSource, /home_medical_transport/);
...
test("only the revocable grant and public device id enter extension storage", () => {
  ...
  assert.doesNotMatch(apiSource, /storageSet\([^)]*(?:patientCharge|patient-charge|billingHandling)/s);
});
