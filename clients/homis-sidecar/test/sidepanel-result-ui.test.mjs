...
test("result UI shows read freshness, four work zones, narrowing, attention, and calculation diff", async () => {
  ...
  const result = await page.evaluate(() => {
    ...
    const exactSelection = [...document.querySelectorAll("#proposal-candidates .candidate-row")]
      .find((node) => node.querySelector(".candidate-name")?.textContent === "在宅患者訪問診療料（区分確定例）");
    const selection = document.querySelector("#selection-candidates .candidate-row");
    const blocked = document.querySelector("#blocked-candidates .candidate-row");
    const href = included.querySelector(".candidate-code").href;
    return {
      ...
      exactSelectionQualifier: exactSelection?.querySelector(".candidate-qualifier")?.textContent,
      ...
      blockedPoints: blocked.querySelector(".candidate-points").textContent,
      ...
      shirobonOrigin: new URL(href).origin,
      shirobonSearch: new URL(href).search,
      shirobonHasPhi: /1001|2026-06-25|患者/u.test(href),
  ...
  assert.deepEqual(result.zoneHeadings, ["算定案に含まれる", "要確認", "要選択", "採用不可"]);
  ...
  assert.equal(result.exactSelectionQualifier, "要確認（区分確定済み）");
  assert.equal(result.exactSelectionCode, "114030310");
  assert.equal(result.selectionPoints, "要選択");
  assert.equal(result.blockedPoints, "要確認");
  ...
  assert.equal(result.shirobonOrigin, "https://shirobon.net");
  assert.equal(result.shirobonSearch, "?s=114030310");
  assert.equal(result.shirobonHasPhi, false);
  assert.deepEqual(result.checklistAttention, ["attention-required", "attention-recommended"]);
  assert.deepEqual(result.detailAttention, ["attention-required", "attention-recommended", "attention-reference"]);
  ...
  assert.equal(await page.locator("#detail-log .detail-log-row").count(), 3);
