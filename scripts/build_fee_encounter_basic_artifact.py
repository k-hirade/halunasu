...
SOURCE_PATH = ROOT / "data/fee-rules/source/encounter-basic-fee-sets-2026.json"
MASTER_DB_PATH = ROOT / "python/data/master/standard-master.sqlite"
OUTPUT_PATH = (
    ROOT
    / "services/fee-api/src/fee-rule-data/encounter-basic-fee-sets-2026.generated.json"
)
...
def main() -> int:
    ...
    if args.check:
        ...
        if current != rendered:
            raise SystemExit(f"fee encounter basic artifact is stale: {args.output}")
        print(f"fee encounter basic artifact is current: {args.output}")
        return 0
...
def build_artifact(source: dict[str, Any], master_db: Path) -> dict[str, Any]:
    ...
    try:
        ...
        for source_rule in source["rules"]:
            rows = connection.execute(
                ...
                ORDER BY code
                """,
                (master_source["id"], source_rule["procedureName"]),
            ).fetchall()
            if len(rows) != 1:
                raise ValueError(
                    "procedure name must resolve uniquely: "
                    f"{source_rule['procedureName']} ({len(rows)})"
                )
            row = rows[0]
            rules.append(
                {
    ...
    payload = {
        "schemaVersion": "fee-encounter-basic-artifact-v1",
        ...
        "ruleBasisUrls": sorted(source["ruleBasisUrls"]),
...
def validate_source(source: dict[str, Any]) -> None:
    if source.get("schemaVersion") != "fee-encounter-basic-source-v1":
        raise ValueError("unsupported fee encounter basic source schema")
    if not source.get("rules"):
        raise ValueError("encounter basic rules are missing")
    ...
    for rule in source["rules"]:
        rule_id = required(rule.get("ruleId"), "rules.ruleId")
        if rule_id in rule_ids:
            raise ValueError(f"duplicate encounter rule: {rule_id}")
        rule_ids.add(rule_id)
        required(rule.get("category"), f"{rule_id}.category")
        required(rule.get("procedureName"), f"{rule_id}.procedureName")
