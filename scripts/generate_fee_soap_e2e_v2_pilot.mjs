#!/usr/bin/env node
// v2パイロット生成器。ケース定義(claim contextが先、カルテは自然文)からデータセットJSONを出力する。
// ゴールド契約: テンプレ/メタ文なし・点数表正式名称なし・施設属性はfixture参照・実施根拠は共通述語準拠。
// 難易度: L1=雑情報少なめ / L2=ディストラクタ(過去・他院・否定・市販薬)混在 / L3=時系列・帰属の混在が濃い。
// distractors = 本文に存在するが算定してはならない情報。抽出側の過剰算定検出力を担保する。
...
const dataset = {
  ...
  version: "2026-06-12.pilot.2",
  purpose: "実カルテに近い文体での診療報酬算定E2Eゴールド第2世代。パイロット10件(難易度・ディストラクタ付き)。",
  goldContract: {
  ...
  cases: cases.map((item) => ({
    ...
    qualityLabel: "synthetic_v2_pilot",
    ...
    evidence: [{ source: "engine_replay", note: "expectedCalculationはclaim-context再生検証で確定。医療事務レビュー前。" }],
...
console.log(`wrote ${dataset.cases.length} cases to ${path.relative(repoRoot, outPath)}`);
