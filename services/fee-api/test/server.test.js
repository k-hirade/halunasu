...
test("同一建物区分で施設恒常算定ルールを選び、未確定時は両バリアントを保留する", async () => {
  const stores = createStores();
  const headers = await signedHeaders(stores.platformStore);
  const receivedInputs = [];
  const sessionIds = [];
  const pointMap = new Map([
    ["114001110", ["在宅患者訪問診療料（１）１（同一建物居住者以外）", 890]],
    ["114030310", ["在宅患者訪問診療料（１）１（同一建物居住者）", 215]],
    ["180725910", ["外来・在宅ベースアップ評価料（１）３（訪問診療時）イ", 79]],
    ["180726010", ["外来・在宅ベースアップ評価料（１）３（訪問診療時）ロ", 19]]
  ]);
  stores.feeCalculator.calculate = async (feeSession, calculationInput) => {
    receivedInputs.push({ feeSession, calculationInput });
    const codes = (calculationInput.calculationOptions?.procedure_codes || [])
      .filter((code) => pointMap.has(code));
    const lineItems = codes.map((code, index) => ({
      lineId: `line_${index + 1}`,
      code,
      name: pointMap.get(code)[0],
      points: pointMap.get(code)[1],
      quantity: 1,
      totalPoints: pointMap.get(code)[1],
      status: "needs_review"
    }));
    return {
      provider: "test",
      source: "test",
      status: "completed",
      totalPoints: lineItems.reduce((sum, line) => sum + line.totalPoints, 0),
      lineItems,
      warnings: []
    };
  };
  const clinicalFactsExtractor = async () => ({
    visit_type: { kind: "home_visit", evidence: "定期訪問", confidence: "high" },
    diagnoses: [],
    clinical_events: [],
    excluded_events: [],
    missing_information: [],
  ...
  const settings = await request(stores, "PATCH", "/v1/fee/settings/fac_001", {
    autoBillingRules: [
      {
        ruleId: "home_visit_fee",
        title: "在宅患者訪問診療料（１）１（同一建物居住者以外）",
        code: "114001110",
        sameBuildingTitle: "在宅患者訪問診療料（１）１（同一建物居住者）",
        sameBuildingCode: "114030310",
        action: "confirm",
        settings: ["home_visit"]
      },
      {
        ruleId: "home_visit_baseup",
        title: "外来・在宅ベースアップ評価料（１）３（訪問診療時）イ",
        code: "180725910",
        sameBuildingTitle: "外来・在宅ベースアップ評価料（１）３（訪問診療時）ロ",
        sameBuildingCode: "180726010",
        action: "confirm",
        settings: ["home_visit"]
      }
    ]
  ...
  assert.equal(settings.statusCode, 200);
  assert.equal(settings.body.settings.autoBillingRules[0].sameBuildingCode, "114030310");

  const patient = await request(stores, "POST", "/v1/fee/patients", { displayName: "同一建物テスト患者" }, headers);
  async function calculate(encounterDetails, serviceDate) {
    const session = await request(stores, "POST", "/v1/fee/sessions", {
      patientId: patient.body.patient.patientId,
      facilityId: "fac_001",
      serviceDate,
      setting: "home_visit",
      encounterDetails,
      clinicalText: "定期訪問。全身状態は安定。"
    }, headers);
    sessionIds.push(session.body.feeSession.feeSessionId);
    return request(
      stores,
      "POST",
      `/v1/fee/sessions/${session.body.feeSession.feeSessionId}/calculate`,
  ...
  const sameBuilding = await calculate({
    sameBuilding: true,
    sameBuildingSource: "dom",
    singleBuildingPatientCount: 4
  }, "2026-06-21");
  const outside = await calculate({
    sameBuilding: false,
    sameBuildingSource: "user",
    singleBuildingPatientCount: 1
  }, "2026-06-22");
  ...
  assert.equal(sameBuilding.body.calculationResult.totalPoints, 234);
  assert.deepEqual(
    receivedInputs[0].calculationInput.calculationOptions.procedure_codes.filter((code) => pointMap.has(code)),
    ["114030310", "180726010"]
  );
  assert.equal(outside.body.calculationResult.totalPoints, 969);
  assert.deepEqual(
    receivedInputs[1].calculationInput.calculationOptions.procedure_codes.filter((code) => pointMap.has(code)),
    ["114001110", "180725910"]
  );
  assert.equal(unknown.body.calculationResult.totalPoints, 0);
  assert.deepEqual(
    (receivedInputs[2].calculationInput.calculationOptions?.procedure_codes || []).filter((code) => pointMap.has(code)),
    []
  );
  assert.ok(unknown.body.calculationResult.warnings.some((warning) => warning.includes("同一建物区分が未確定")));
  assert.equal(unknown.body.calculationResult.inputSnapshot.encounterDetails.sameBuilding, null);
  const unresolvedTrace = unknown.body.calculationResult.clinicalExtraction.trace.find((entry) => (
    entry.stage === "facility_auto_billing_rule" && entry.reason === "same_building_unknown"
  ));
  assert.ok(unresolvedTrace);
  ...
  assert.equal(resetToUnknown.statusCode, 200);
  assert.equal(resetToUnknown.body.feeSession.calculationResult, null);
  const recalculatedUnknown = await request(
    stores,
    "POST",
    `/v1/fee/sessions/${sessionIds[1]}/calculate`,
    {},
    headers,
    { clinicalFactsExtractor }
  );
  assert.equal(recalculatedUnknown.body.calculationResult.totalPoints, 0);
  assert.deepEqual(
    (receivedInputs[3].calculationInput.calculationOptions?.procedure_codes || []).filter((code) => pointMap.has(code)),
    []
  );
