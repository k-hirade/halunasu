...
export const FEE_CLINICAL_RULE_SET_VERSION = "fee-clinical-rules-v5";
...
function reviewIssueFromManagementClinicalEvent(event = {}, { categoryLabel = "医学管理等" } = {}) {
  const name = clinicalEventName(event);
  const title = name ? `管理料確認: ${name}` : `管理料確認: ${categoryLabel}`;
  const policy = DERIVED_BILLING_ITEM_POLICIES.management_fee;
  const messageForStaff = `管理料確認: ${name || categoryLabel}は管理料・指導料に関係する記載として抽出しました。同月履歴確認、対象疾患、指導・説明の記録、管理主体、施設基準を確認してから採用してください。自動算定には入れていません。`;
  return {
